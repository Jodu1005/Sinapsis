...
- Agent 被要求只在该 worktree 内读写，可运行测试并创建提交；这不是操作系统权限限制。
- Agent 完成后必须在任务分支上留下提交，才可进入待人工验收。
- 内置 API 不会自动推送远端或合并目标分支；验收只记录审查决定，不会改变远端或目标分支。

### 本机安全边界

- 第一版没有 OS 级沙箱。worktree 是协作隔离约定，不是文件系统、网络或凭据的权限边界。
- 运行本机 CLI 前，用户必须先确认信任它。未经信任的 CLI 仍可能访问当前 OS 用户可见的文件、网络和凭据，或显式提供自己的凭据。
- `ProcessRunner` 只继承 `PATH`、`HOME`、语言/时区与临时目录变量；继承的 Git、SSH、CI 与常见远端凭据变量不会传入 Runtime。
- `ProcessRunner` 强制 Git 非交互、忽略全局 credential helper，并让 SSH 使用 batch mode。显式配置的 Agent profile 环境变量仍会传入；环境过滤降低意外继承，不构成对不受信任 Agent 的沙箱或凭据阻止。
...
- Agent 被要求在自己的 task worktree 内读写、运行测试和创建提交；第一版不在 OS 层强制这一约定。
- 内置 API 不提供自动 `git push` 或合并。对工作树外删除、网络访问和其他外部副作用，用户应只运行自己信任的本机 CLI，而不能把人工确认视为强制阻止。
