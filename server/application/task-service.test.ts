...
describe('task API', () => {
  ...
  it('lists a repository task and keeps human input in its structured input queue', async () => {
    ...
    await expect(detailResponse.json()).resolves.toMatchObject({
      task: { id: task.id },
      inputs: [{ body: '数据库使用 SQLite。', consumedAt: null }],
      sessions: [], leases: [], decisions: [], artifacts: [],
    })
  })
