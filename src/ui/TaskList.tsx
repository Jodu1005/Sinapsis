...
const filters = [
  ['backlog', '积压'], ['waiting', '待办'], ['running', '执行中'], ['waiting_input', '等待输入'], ['in_review', '待人工验收'], ['ended', '已结束'],
] as const
...
function matchesFilter(task: TaskView, filter: (typeof filters)[number][0]): boolean {
  if (filter === 'backlog') return task.status === 'backlog' || task.status === 'needs_human'
  ...
  return ['completed', 'accepted', 'returned', 'needs_human', 'merged', 'cancelled'].includes(task.status)
}
