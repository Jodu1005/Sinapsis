...
export function AgentStatusList({ agents, onSelect, onCreate }: { agents: AgentView[]; onSelect(agent: AgentView): void; onCreate(): void }) {
  return <section className="agent-status-list" aria-label="全局 Agent">
    <div className="sidebar-section-label sidebar-section-heading"><span>全局 Agent</span><button className="icon-button" type="button" aria-label="添加 Agent" data-tooltip="添加 Agent" onClick={onCreate}><Plus size={15} /></button></div>
