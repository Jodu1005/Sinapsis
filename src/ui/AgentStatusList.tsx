...
export function AgentStatusList({ agents }: { agents: AgentView[] }) {
  return <section className="agent-status-list" aria-label="Agent 状态">
    <div className="sidebar-section-label">运行中的 Agent</div>
    {agents.length === 0 ? <p className="sidebar-empty">尚未添加 Agent</p> : agents.map((agent) => <div className="agent-status" key={agent.id}>
