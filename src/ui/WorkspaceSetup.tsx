...
export function WorkspaceSetup({ api, onComplete }: { api: WorkspaceApi; onComplete(): Promise<void> }) {
  ...
  return <main className="workspace-setup"><section className="setup-content"><span className="setup-symbol"><Sparkles size={23} /></span><h1>创建工作空间</h1><p>先绑定一个本地工作目录，再让 Agent 在清晰的频道里协作。</p><form onSubmit={submit} className="setup-form">
    ...
    <label htmlFor="workspace-directory">工作目录</label><div className="path-input"><FolderPlus size={18} /><input id="workspace-directory" aria-label="工作目录" value={directory} onChange={(event) => setDirectory(event.target.value)} placeholder="/Users/you/Projects/app（可稍后添加）" /></div>
