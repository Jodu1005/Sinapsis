...
export function MessageComposer({ channelName, agents, onSend }: { channelName: string; agents: AgentView[]; onSend(body: string): Promise<{ notice?: string } | void> }) {
  ...
  useEffect(() => { setActiveSuggestionIndex(0) }, [mention?.query, mention?.start])
  ...
  const selectAgent = (agent: AgentView, selection = mention) => {
    if (!selection) return
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (selected) {
      selectAgent(selected, mentionAtCaret(body, event.currentTarget.selectionStart))
      return
    }
  ...
  return <form ref={formRef} className="message-composer" onSubmit={submit}>
    ...
    {mention && suggestions.length > 0 && <div className="mention-suggestions" role="listbox" aria-label="可提及 Agent">{suggestions.map((agent, index) => <button key={agent.id} type="button" role="option" aria-selected={index === activeSuggestionIndex} onMouseDown={(event) => event.preventDefault()} onClick={() => selectAgent(agent)}><strong>@{agent.identity}</strong><small>{agent.runtime} · {statusLabel(agent.status)}</small></button>)}</div>}
