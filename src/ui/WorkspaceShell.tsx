import { PanelRightClose, PanelRightOpen, Settings2, X } from 'lucide-react'
import { useCallback, useEffect, useMemo, useState } from 'react'
import type { WorkspaceApi } from '../api/client'
import { ApiClient } from '../api/client'
import { useWorkspaceEvents } from '../api/use-workspace-events'
import { channelMessages, taskStatusLabel, type TaskView, type WorkspaceSnapshot, type WorkspaceView } from '../domain/workspace-view'
...
export function WorkspaceShell({ api: providedApi }: { api?: WorkspaceApi }) {
  ...
  const selectedTask = useMemo(() => findTask(workspace, selectedTaskId) ?? selection.repository?.tasks.find((task) => task.channelId === selection.channel?.id) ?? null, [workspace, selectedTaskId, selection])
  ...
  const selectChannel = (channelId: string) => { setSelectedChannelId(channelId); setSelectedTaskId(null); setNavOpen(false) }
  return <div className="workspace-shell">
    <RepositorySidebar workspace={workspace} selectedChannelId={selection.channel.id} onSelectChannel={selectChannel} mobileOpen={navOpen} onClose={() => setNavOpen(false)} />
    ...
    <aside className="context-panel" aria-label="任务与上下文" data-mobile-open={contextOpen}>
      ...
      <section className="context-section"><h2>本频道任务</h2>{selection.repository.tasks.filter((task) => task.channelId === selection.channel!.id).length === 0 ? <p className="context-empty">没有关联任务</p> : <div className="task-list">{selection.repository.tasks.filter((task) => task.channelId === selection.channel!.id).map((task) => <button type="button" key={task.id} className="task-row" aria-pressed={selectedTask?.id === task.id} onClick={() => setSelectedTaskId(task.id)}><span>{task.title}</span><small>{taskStatusLabel(task.status)}</small></button>)}</div>}</section>
      {selectedTask && <TaskContext task={selectedTask} />}
      <section className="context-section"><h2>频道操作</h2><button type="button" className="context-action"><Settings2 size={16} /> 设置频道</button><button type="button" className="context-action"><PanelRightClose size={16} /> 收起上下文</button></section>
    </aside>
  </div>
}
