...
export function AgentConfigDialog({ agent, refreshingRuntime, onRefreshRuntime, onClose }: { agent: AgentView; refreshingRuntime: boolean; onRefreshRuntime(): Promise<void>; onClose(): void }) {
  ...
  return <div className="panel-scrim" role="presentation"><section ref={dialogRef} className="agent-config-dialog" role="dialog" aria-modal="true" aria-labelledby="agent-config-title"><header><div><p>Agent 配置</p><h2 id="agent-config-title">{agent.identity}</h2></div><div><button className="icon-button" type="button" aria-label="重新检测 Agent Runtime" data-tooltip="重新检测 Agent Runtime" onClick={() => void onRefreshRuntime()} disabled={refreshingRuntime}><RefreshCw size={18} /></button><button ref={closeButtonRef} className="icon-button" type="button" aria-label="关闭 Agent 配置" data-tooltip="关闭" onClick={onClose}><X size={18} /></button></div></header>
