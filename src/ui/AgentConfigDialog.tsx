...
export function AgentConfigDialog({ agent, onClose }: { agent: AgentView; onClose(): void }) {
  return <div className="panel-scrim" role="presentation"><section className="agent-config-dialog" role="dialog" aria-modal="true" aria-labelledby="agent-config-title"><header><div><p>Agent 配置</p><h2 id="agent-config-title">{agent.identity}</h2></div><button className="icon-button" type="button" aria-label="关闭 Agent 配置" data-tooltip="关闭" onClick={onClose}><X size={18} /></button></header>
