...
- 新增独立 `MemoryConsolidator`，每次 Dream 使用单个注入的 RuntimeAdapter，在 `${dataDir}/dream/<runId>` 启动 task request，不设置 conversation metadata，不复用或恢复频道 session。
...
- Runtime artifact 仅写入 run 目录的本机证据文件，不进入 Candidate 或频道消息。跨频道 message 在创建目录和启动 Runtime 前直接拒绝。
...
- 最终专项：`npm test -- --run server/application/memory-consolidation-protocol.test.ts server/application/memory-consolidator.test.ts server/config.test.ts`，3 个文件、48/48 通过。
- 最终全量：`npm test -- --run`，54 个文件、529/529 通过。
